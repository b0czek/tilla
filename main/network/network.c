...
const char *NETWORK_TAG = "network_connect";
                      ...
                      int32_t event_id, void *event_data)
{
    ...
    if (!is_our_netif(NETWORK_TAG, event->esp_netif))
    {
        ESP_LOGW(NETWORK_TAG, "Got IPv4 from another interface \"%s\": ignored", esp_netif_get_desc(event->esp_netif));
        return;
    }
    ...
    ESP_LOGI(NETWORK_TAG, "Got IPv4 event: Interface \"%s\" address: " IPSTR, esp_netif_get_desc(event->esp_netif), IP2STR(&event->ip_info.ip));
                        ...
                        int32_t event_id, void *event_data)
{
    ...
    if (!is_our_netif(NETWORK_TAG, event->esp_netif))
    {
        ESP_LOGW(NETWORK_TAG, "Got IPv6 from another netif: ignored");
        return;
    }
    esp_ip6_addr_type_t ipv6_type = esp_netif_ip6_get_addr_type(&event->ip6_info.ip);
    ESP_LOGI(NETWORK_TAG, "Got IPv6 event: Interface \"%s\" address: " IPV6STR ", type: %s", esp_netif_get_desc(event->esp_netif),
...
esp_err_t network_connect(void)
{
    ...
    ESP_LOGI(NETWORK_TAG, "Waiting for IP(s)");
    ...
    for (int i = 0; i < esp_netif_get_nr_of_ifs(); ++i)
    {
        ...
        if (is_our_netif(NETWORK_TAG, netif))
        {
            ESP_LOGI(NETWORK_TAG, "Connected to %s", esp_netif_get_desc(netif));
            ESP_ERROR_CHECK(esp_netif_get_ip_info(netif, &ip));

            ESP_LOGI(NETWORK_TAG, "- IPv4 address: " IPSTR, IP2STR(&ip.ip));
#ifdef CONFIG_CONNECT_IPV6
            ...
            for (int j = 0; j < ip6_addrs; ++j)
            {
                ...
                ESP_LOGI(NETWORK_TAG, "- IPv6 address: " IPV6STR ", type: %s", IPV62STR(ip6[j]), s_ipv6_addr_types[ipv6_type]);
            }
#endif
        }
    }
...
static void on_wifi_disconnect(void *arg, esp_event_base_t event_base,
                               int32_t event_id, void *event_data)
{
    ESP_LOGI(NETWORK_TAG, "Wi-Fi disconnected, trying to reconnect...");
...
static esp_netif_t *wifi_start(void)
{
    ...
    // Prefix the interface description with the module NETWORK_TAG
    // Warning: the interface desc is used in tests to capture actual connection details (IP, gw, mask)
    asprintf(&desc, "%s: %s", NETWORK_TAG, esp_netif_config.if_desc);
    ...
    ESP_LOGI(NETWORK_TAG, "Connecting to %s...", wifi_config.sta.ssid);
...
static void wifi_stop(void)
{
    esp_netif_t *wifi_netif = network_get_netif_from_desc("sta");
...
static void eth_stop(void)
{
    esp_netif_t *eth_netif = network_get_netif_from_desc("eth");
...
esp_netif_t *network_get_netif_from_desc(const char *desc)
{
    esp_netif_t *netif = NULL;
    char *expected_desc;
    asprintf(&expected_desc, "%s: %s", NETWORK_TAG, desc);
