...
json_handler_auth(handle_device_restart, device_restart);
...
esp_err_t register_device_handlers(httpd_handle_t server)
{
    ...
    result += httpd_register_uri_handler(server, &device_info_uri);

    httpd_uri_t chip_info_uri = {
        .uri = "/api/v1/device/chip/?",
        .method = HTTP_GET,
        .handler = handle_chip_info};
    result += httpd_register_uri_handler(server, &chip_info_uri);

    httpd_uri_t network_info_uri = {
        .uri = "/api/v1/device/network/?",
        .method = HTTP_GET,
        .handler = handle_network_info};
    result += httpd_register_uri_handler(server, &network_info_uri);

    httpd_uri_t stats_info_uri = {
        .uri = "/api/v1/device/stats/?",
        .method = HTTP_GET,
        .handler = handle_stats_info};
    result += httpd_register_uri_handler(server, &stats_info_uri);

    httpd_uri_t restart_device_uri = {
        .uri = "/api/v1/device/restart/?",
        .method = HTTP_GET,
        .handler = handle_device_restart};
    result += httpd_register_uri_handler(server, &restart_device_uri);
